...
8.  **IPA must match the cell**: The `from`/`to` sounds in your examples must realise the *same phonemes* as `fromId`/`toId`. If the evidence you find documents a neighbouring but distinct sound, that is evidence for a *different* cell — mark this one `{"unattested": true}` rather than stretching the example to fit. Watch especially for:
    ...
    *   **rhotic/glide vocalization targets a specific vowel**: coda /r/ (including a flap [ɾ]) vocalizes to a **central or back lax** vowel, not an arbitrary front vowel. German and English coda-/r/ vocalization yields [ɐ] / schwa (`Tor` → [toːɐ]), so it belongs in the `_to_schwa`/`_to_a` cells, **not** `r_flap_to_e` or `_to_i`. Do not file a vocalization under a front-vowel target unless the source explicitly gives that front-vowel reflex.
...
11. **Source relevance check**: Before including a source, write (internally) one sentence stating what *that specific source* says about *this transformation in this language*. If you cannot write that sentence, the source does not support the claim — do not include it. A source about Japanese romanisation in Latvian is not a source on Latvian sound change; a source about Baghdad dialects is not a source on Mosul Arabic; a paper on Hittite *ḫ* is not a source for a Germanic *w → ɣ* example. The source must attest the **outcome** of the shift, not just the input sound: a paper documenting *that the input sound exists or varies* (e.g. "Bavarian r-flapping") is **not** a source that the input becomes the `toId` sound (e.g. `r → [e]`) unless it explicitly states that result. If the source names neither the language nor *both* sounds (`from` and `to`) of your examples, drop it — and if that was your only source, output `{"unattested": true}`.
...
14. **Consistent notation within a pair**: The `from` and `to` of a single example must use the *same* notation system. Do not pair an IPA/Latin `from` with a Cyrillic-orthography `to` (e.g. `*nesʲeši` → `несёшь`). Either give both in IPA (`[nʲɪˈsʲeʂ]`) or both in the same orthography. Cross-script pairs are inconsistent and will be rejected.

15. **One real, specific language per entry**: `language` must be a single attested language or a named, real dialect of it. NEVER concatenate two unrelated language names or invent a label like `"Eastern Toda (Seediq dialect)"` — Toda (Dravidian) and Seediq (Austronesian) are unrelated languages, not a language-and-dialect. If you are not certain a named dialect exists, give only the language, or output `{"unattested": true}`.

16. **Deletion cells (`toId` = "empty")**: The `to` form must be the `from` form with ONLY the deleted sound removed — every other segment stays identical. The deleted IPA symbol must appear in `from` and be absent from `to`. An example where the target sound still appears in `to` (e.g. `fʁomaʒ` → `fʁoma`, which keeps `ʁ`), or where unrelated segments also change (e.g. `ʒiʁaf` → `ijap`), does not demonstrate the deletion — drop it.
...
5. **No placeholder text or non-academic sources**: Never write "Source via search", "verify before merge", "research snippets", "Cited in search result N", `(n.d.)`, or any phrase that signals "I have no real citation." Never cite Reddit, Quora, Stack Exchange, **the Straight Dope / "Factual Questions", WordReference, AnswerBag, or any other discussion forum / message-board thread**, Wikipedia, **Grokipedia, Wiktionary, NamuWiki, Fandom or any other AI-generated or crowd-sourced wiki**, **YouTube**, languagehat.com, Scribd, Calaméo, Internet Archive, **dokumen.pub, dokumen.tips**, or any other document-scraping/piracy site — these are not peer-reviewed sources. If your only citation would be one of these, mark `{"unattested": true}`.

6. **Cite the paper, not the platform or a fragment**: ResearchGate, Academia.edu, and Semantic Scholar are *hosting mirrors*, not sources — cite the underlying `Author (Year). Title. Venue.`, and never let the word "ResearchGate"/"Academia.edu" appear in a citation. A citation must be a self-contained reference, **never** a figure/table caption (e.g. `"Murcian diphthongs (white squares: data from Monroy 2008)..."`) or a truncated search-result title (anything containing `...`). Such fragments often carry an incidental year and will otherwise pass the year check — drop them. If a fragment is your only "source", mark `{"unattested": true}`.

7. **No duplicate citations** within a single transformation's `sources` array.
